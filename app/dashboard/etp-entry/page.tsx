...
import { Calculator, Check, Send, Droplets, Zap, Trash2, Lock, TriangleAlert, Ban } from "lucide-react";
...
// capped at 5 integer digits + 1 decimal (Fateh: "up to one decimal place").
...
export default function EtpEntryPage() {
  ...
  const prior = useMemo(() => (industryId ? latestEntryFor(etpEntries, industryId) : undefined), [etpEntries, industryId]);
  ...
  const rowsFor = (keys: readonly { key: string; label: string }[], state: Record<string, MeterState>) =>
    keys.map((m) => {
      ...
      const initial = num(st.initial);
      const final = num(st.final);
      const hasBoth = st.initial !== "" && st.final !== "";
      const invalid = hasBoth && final < initial;
      return { ...m, total: Math.max(0, final - initial), invalid };
    });

  const waterRows = rowsFor(WATER_METERS, water);
  const energyRows = rowsFor(ENERGY_METERS, energy);
  const waterGrandTotal = waterRows.reduce((s, r) => s + r.total, 0);
  const anyMeterInvalid = [...waterRows, ...energyRows].some((r) => r.invalid);

  const totalOf = (key: string) => waterRows.find((r) => r.key === key)?.total ?? 0;
  const totalWaterIntake = totalOf("rawFreshWater") + totalOf("etpTreatedReuse") + totalOf("roPermeate");

  const sludgeClosing = sludge.opening + num(sludge.generation) - num(sludge.dispatch);
  const saltClosing = salt.opening + num(salt.generation) - num(salt.dispatch);
  const sludgeNeedsManifest = num(sludge.dispatch) > 0 && (!sludge.manifestNo.trim() || !sludge.dateOfDisposal);
  const saltNeedsManifest = num(salt.dispatch) > 0 && (!salt.manifestNo.trim() || !salt.dateOfDisposal);
  ...
  // Warn as cumulative dispatch approaches the authorised HW quantity.
  const authQty = industry?.authorisedQuantityKg ?? 0;
  const projectedDispatch = industryId
    ? cumulativeDispatch(etpEntries, industryId, "sludge") + cumulativeDispatch(etpEntries, industryId, "salt") + num(sludge.dispatch) + num(salt.dispatch)
    : 0;
  const authWarn = authQty > 0 && projectedDispatch >= authQty * SLUDGE_WARN_RATIO;
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="One daily sheet: water (10 meters), energy (3 meters) and the sludge / MEE-salt ledgers. Totals are auto-calculated and sent for verification."
      />
      ...
      <div className="rounded-2xl border border-border bg-card p-4 sm:p-6">
        ...
        <div className="mt-4 grid gap-4 lg:grid-cols-2">
          <LedgerCard
            title="ETP Sludge"
            hint="Day-wise stock, in kg. Closing = Opening + Generation − Dispatch."
            state={sludge}
            setState={setSludge}
        ...
        {authWarn && (
          <div className="mt-3 flex items-start gap-2 rounded-xl border border-amber-500/40 bg-amber-500/10 p-3 text-sm text-amber-600">
            <TriangleAlert className="mt-0.5 h-4 w-4 shrink-0" />
            <span>Cumulative dispatch is approaching the authorised quantity ({formatNumber(authQty)} kg). Review before dispatching more.</span>
          </div>
        )}
      </div>
        ...
        {blocked && (
          <div className="mt-4 flex items-start gap-2 rounded-xl border border-red-500/40 bg-red-500/10 p-3 text-sm text-red-600">
            ...
            <span>
              ...
              {anyMeterInvalid && "A Final reading is less than its Initial reading. "}
...
function MeterSection({
  ...
  rows: { key: string; total: number; invalid: boolean }[];
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-4 sm:p-6">
        ...
        <table className="w-full min-w-[560px] border-collapse text-sm">
          <thead>
            <tr className="text-left text-xs uppercase tracking-wide text-muted-foreground">
              ...
              <th className="w-28 pb-2 pr-3 font-medium">Initial</th>
              <th className="w-28 pb-2 pr-3 font-medium">Final</th>
      ...
      {rows.some((r) => r.invalid) && <p className="mt-2 text-xs text-red-500">Final reading cannot be less than Initial reading.</p>}
      <div className="mt-3">
  ...
  return (
    <div className="rounded-xl border border-border bg-muted/20 p-4">
      ...
      <div className="mt-3 grid gap-3 sm:grid-cols-2">
        <Field label="Opening Balance (kg · carried)">
          <div className={readonlyCls}>{formatNumber(state.opening)} kg</div>
        </Field>
        <Field label="Generation (kg)">
        ...
        <Field label="Dispatch / Disposal (kg)">
        ...
        <Field label="Closing Balance (kg · auto)">
